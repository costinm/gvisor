...
func isMLDValid(pkt *stack.PacketBuffer, iph header.IPv6, hasRouterAlert bool, routerAlertValue header.IPv6RouterAlertValue) bool {
	...
	//
	// Note that neither RFC 2710 nor RFC 3810 (MLDv2) explicitly require the
	// Router Alert value to be set to MLD. However Linux checks that it is:
	// https://github.com/torvalds/linux/blob/f40ddce885934/net/ipv6/mcast.c#L1362
	if !hasRouterAlert || routerAlertValue != header.IPv6RouterAlertMLD {
		return false
	}
	if pkt.Data.Size()-header.ICMPv6HeaderSize < header.MLDMinimumSize {
		return false
	}
	if iph.HopLimit() != header.MLDHopLimit || !header.IsV6LinkLocalAddress(iph.SourceAddress()) {
		return false
	}
	return true
}

func (e *endpoint) handleICMP(pkt *stack.PacketBuffer, hasFragmentHeader, hasRouterAlert bool, routerAlertValue header.IPv6RouterAlertValue) {
	...
	case header.ICMPv6MulticastListenerQuery, header.ICMPv6MulticastListenerReport, header.ICMPv6MulticastListenerDone:
		...
		if !isMLDValid(pkt, iph, hasRouterAlert, routerAlertValue) {
			received.invalid.Increment()
			return
		}
